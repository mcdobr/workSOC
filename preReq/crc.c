...
{
  /**
   * data este valoarea la adresa curenta
   * crc este rezultatul operatiei
   */
                              ...
                              unsigned int len)
{
  ...
  for (i = 0; i < len; ++i) {
    table_index = ((crc >> 8) ^ *(__flash char *)addr_start++) & 0xFF;
							   ...
							   unsigned int len)
{
	...
	for (i = 0; i < len; ++i) {
		table_index = ((crc >> 24) ^ *(__flash char*)addr_start++) & 0xFF;
...
__flash const unsigned int crc16_table[256]= {
...
__flash const unsigned long crc32_table[256] = {
